...
DaVinci Resolve の Fusion には`Expression`として計算式を設定できますが
...
幅を求めたいので三角関数のマイナス値側はいらないです、`abs`を使って絶対値に変換します
...
こちらは`Rectangle1.Height`をもとに計算するので、`アスペクト比: self:GetData("aspectRatio")`の逆数をかけます
:::message
こちらもアスペクト比をかけるので結果的に打ち消されます
:::

#### 結果

これらを足して、`高さ`を求める式は 👇 こちらです 😤
