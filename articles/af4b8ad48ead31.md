...
`FFmpeg`は動画や音声の変換や切り出しなど幅広い処理に対応した、とても有名なソフトウェアです。無料で利用できます💰👌

## Whisper(whisper.cpp)とは？

https://github.com/ggml-org/whisper.cpp

OpenAIが作った音声認識AIモデル🤖がWhisperです。このモデルを使うと音声をテキスト化できます。それをC++で実装したものがwhisper.cppで、こちらも無料です💰👌
...
ここからは準備です。難しいことはありませんが、少しだけ段取りが必要です🙏  
文字起こしを行うには`FFmpeg`と`whisper.cpp`のモデルを用意します。どちらも🌐インターネットからダウンロードできます！
...
今回はファイル名に`.en`が付くものと、`-q5_0`や`-tdrz`が付くものは見送ります🫡  
よって以下の3つから選びます。サイズが大きいほど文字起こし精度も高い傾向があります(多分)🫠
...
:::message
どれか1つあれば十分ですが、余裕があるなら3つとも準備しておくと便利です。さらに余裕があるなら`-q5_0`などの量子化モデルも用意しておくと🙆‍♀️
:::
...
文字起こしが完了すると`srt`ファイルが作成されます。これは字幕ファイルと呼ばれるテキストファイルで、📝テキストエディタで開けます。
...
気になる場合は✋手作業で修正するか、🤖AIに手伝ってもらいましょう。どちらの場合でも修正するのは __内容__ の部分だけにしてください。それ以外を変更すると、他のアプリで読み込めなくなる可能性があります。

:::message
テキストの区切り方が不自然に感じられる場合は、`queue=`の値を大きくすると自然な区切りに近づきます(多分🙈)。  
サンプルファイルでは20より100の方がきれいに感じました💦💦
...
登録した`srt`ファイルを`タイムライン`にドラッグ&ドロップします。
...
この方法のポイントはPythonのインストールやスクリプト作成が不要な点です。プログラミングの知識がなくても取り組めます！！

「これができるなら、文字起こしのためにDaVinci Resolve有料版(DaVinci Resolve Studio)は要らないのでは？」と思うかもしれませんが、有料版には文字起こし結果と連動したテキストベース編集など💪強力で💃魅力的な機能が備わっています。今回の内容で完全に置き換えられるわけではない点だけご注意ください。
